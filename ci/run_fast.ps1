...
foreach ($v in $targets) {
    ...
    $t0 = Get-Date
    docker run --rm -v "${dir}:C:\workspace" us-docker.pkg.dev/perl-cloud-ci/perl-cloud-ci-images/google-cloud-perl-ci-windows:${v} cmd /c "cd /d C:\workspace && set CI_SKIP_DEPS=1 && perl .github/scripts/build_and_test.pl Google-Auth"
    $elapsed = (Get-Date) - $t0
