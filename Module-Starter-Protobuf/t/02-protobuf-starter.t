...
ok(-f $starter_script, 'protobuf-starter script exists');

# 2. Run protobuf-starter CLI to generate the client distribution
...
my $starter_cmd = sprintf(
    '"%s" "%s" --module=Google::Cloud::Test --protos="%s" --import-path="%s" --grpc-target=test.googleapis.com --dir="%s" --author="C.J. Collier <cjac@google.com>" --force',
    $^X,
    $starter_script,
    $proto_file,
    $import_path,
...
my $rc_starter = system($starter_cmd);
...
open my $fh_runner, '>', $runner_file or die 'Failed to write run-integration-test-starter.pl: ' . $!;
print {$fh_runner} <<'EOF';
use strict;
use warnings;
...
local $ENV{TEMPLATE_STASH} = 'Template::Stash';
