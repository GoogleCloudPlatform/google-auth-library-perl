...
sub get_credentials {
  ...
  my $creds = Google::Auth::UserRefreshCredentials->new(
    ...
    expires_at => $data->{expiration_time_millis} ? $data->{expiration_time_millis} / 1000 : undef,
  );
...
sub _redirect_uri_for {
  ...
  return $self->callback_uri if $self->callback_uri eq 'urn:ietf:wg:oauth:2.0:oob';
  return $self->callback_uri if $self->callback_uri eq 'postmessage';
  
  if (!$base_url) {
     Google::Auth::Error->throw('Absolute base url required for relative callback url');
  }
  
  my $uri = URI->new($base_url);
  # Resolve relative path
  $uri->path($self->callback_uri); # This might overwrite path completely if callback_uri starts with /
  return $uri->as_string;
}
