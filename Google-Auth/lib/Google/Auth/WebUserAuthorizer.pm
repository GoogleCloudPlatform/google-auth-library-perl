...
sub handle_auth_callback_deferred {
  my ($class, $request) = @_;
  my ($callback_state, $redirect_uri) = $class->extract_callback_state($request);
...
sub validate_callback_state {
  ...
  if ($state->{+ERROR_CODE_KEY}) {
     Google::Auth::Error->throw('Authorization error: ' . $state->{+ERROR_CODE_KEY});
  }
  ...
  if (!defined $stored_xsrf || !defined $received_xsrf || $stored_xsrf ne $received_xsrf) {
