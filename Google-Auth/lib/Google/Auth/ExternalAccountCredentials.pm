...
around BUILDARGS => sub {
  ...
  if (my $json = $args->{json_key}) {
    ...
    $args->{service_account_impersonation_url} //=
...
sub make_creds {
  ...
  if (defined $subject_token_type
    && $subject_token_type eq 'urn:ietf:params:aws:token-type:aws4_request')
  {
...
sub retrieve_subject_token {
  ...
  if (defined $source->{file}) {
    ...
    if (!-f $source_name) {
      Google::Auth::Error->throw(
        'Credential source file ' . $source_name . ' not found');
    }
  ...
  if ($format_type eq 'text') {
    ...
    $log->tracef(
      'Parsing OIDC JSON response to extract subject token field: %s', $field);
    my $data = eval { decode_json($content) };
    if ($@) {
      $log->errorf('OIDC JSON parsing failed: %s', $@);
      Google::Auth::Error->throw(
        'Failed to parse JSON from credential resource ' .
          $source_name . ': ' .
          $@);
    }
    my $token = $data->{$field};
    if (!defined $token) {
      ...
      Google::Auth::Error->throw(
        'Missing field ' . $field . ' in OIDC JSON response');
    }
...
sub fetch_access_token {
  ...
  if ($self->service_account_impersonation_url) {
    $log->infof(
'Service account impersonation URL set. Requesting delegated credentials via %s...',
